...
app.get("/coreui-free-react-admin-template-master/Login.test", function(req, res){
	res.render("/coreui-free-react-admin-template-master/Login.test");
});

/*app.post("/coreui-free-react-admin-template-master/Login.test", function(req, res){
	...
	if(	secret == "nodejsogreniyorum" ){
		...
		res.redirect("/coreui-free-react-admin-template-master/Login.test");
	}
});*/

app.get("//coreui-free-react-admin-template-master/giris", function(req, res){
	res.render("coreui-free-react-admin-template-master/Login");
});

app.post("/coreui-free-react-admin-template-master/giris", function(req, res){
	//var coreui-free-react-admin-template-masterID = req.body.id;
	// console.log(adminID);
	if( app.get('id') == coreui-free-react-admin-template-masterID ){
		User.find({}, (err, usersDB) => {
			res.render("coreui-free-react-admin-template-master/Login", {users:usersDB});
		});
	} else {
		res.redirect("/yemekler");
	}
...
app.get("/dil",function (req, res) {
	res.writeHead(200, {'Content-Type': 'text/plain'});
	mrb.textContent=language.es.welcome;
	res.render('/dil');
